...
const successLogic = (data) => {
  if(data.data.length > 0){
    ...
    data.data.map((acc) => {
      ...
      <option value="${acc.accountNumber}" ${acc.status==='dormant'?'disabled':'enabled'}>${acc.accountNumber}-(${acc.type})-${acc.status}</option>
      `;
...
if (trasactButton) {
  trasactButton.onclick = () => {
    if (amountTextField.value === '' || amountTextField.value === null) {
      ...
      const transactUrl = `${baseUrl}/transactions/${accSpinner.value}/${transactType.value}`;
